...
main_url = f"{base_url}/products/memory/"
driver.get(main_url)
...
ram_rows = soup.find_all("tr", class_="tr__product")[:]  # First 30 RAMs
...
for product in products:
    ...
    try:
        ...
        for key, label in spec_mapping.items():
            try:
                spec_element = detail_soup.find("h3", string=lambda t: label in str(t))
                if spec_element:
                    content_div = spec_element.find_next_sibling("div")
                    if content_div:
                        # Handle different content types
                        if content_div.find("p"):
                            spec_value = content_div.find("p").get_text(strip=True)
                ...
                print(f"Error extracting {key} for {product['name']}: {str(e)}")
                specs[key] = "N/A"
