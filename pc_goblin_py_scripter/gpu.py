...
main_url = f"{base_url}/products/video-card/"
driver.get(main_url)
...
gpu_rows = soup.find_all("tr", class_="tr__product")[:]  # First 4 GPUs
...
for product in products:
    ...
    try:
        ...
        specs = {}
        try:
            spec_groups = detail_soup.find_all("div", class_="group group--spec")
            for group in spec_groups:
                title_tag = group.find("h3", class_="group__title")
                value_tag = group.find("div", class_="group__content")
                if title_tag and value_tag:
                    label = title_tag.get_text(strip=True)
                    # Value could be in a <p> or a <ul><li> list
                    if value_tag.find("ul"):
                        value = ", ".join(li.get_text(strip=True) for li in value_tag.find_all("li"))
                    else:
                        value = value_tag.get_text(strip=True)
                    specs[label.lower().replace(" ", "_")] = value
        except Exception as e:
            print(f"Spec parsing error for {product['name']}: {str(e)}")
        ...
        # Only add to results if price is available
        if price != "N/A":
            ...
            print(f"Skipped {product['name']} - No price available")
...
print(f"Saved to gpu_data.json")
...
try:
    ...
    batch = db.batch()
    docs = db.collection("GPUs").stream()
    for doc in docs:
    ...
    batch = db.batch()
    for gpu in results:
        new_ref = db.collection("GPUs").document()
        batch.set(new_ref, gpu)
    batch.commit()
