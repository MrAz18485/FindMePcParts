...
main_url = f"{base_url}/products/cpu/"
driver.get(main_url)
...
cpu_rows = soup.find_all("tr", class_="tr__product")[:]  # First 38 CPUs
...
for product in products:
    ...
    try:
        ...
        spec_mapping = {
            ...
            "core_clock": "Performance Core Clock",
            "boost_clock": "Performance Core Boost Clock",
            "l2_cache": "L2 Cache",
            "l3_cache": "L3 Cache",
            "tdp": "TDP",
            "cooler_included": "Includes CPU Cooler"
        }
        ...
        for key, label in spec_mapping.items():
            try:
                ...
                # Clean up TDP value by removing 'W' if present
                if key == "tdp" and 'W' in spec_value:
                    spec_value = spec_value.replace('W', '').strip()
